...
function adfoin_kit_settings_view(  $current_tab  ) {
    ...
    $instructions = sprintf( '<p>%s</p>', __( 'Go to Account Settings > Developer. Create and copy V4 API Key', 'advanced-form-integration' ) );
    ADFOIN_Account_Manager::render_settings_view(
        'kit',
        __( 'Kit', 'advanced-form-integration' ),
        $fields,
        $instructions
    );
}
...
add_action(
    'wp_ajax_adfoin_get_kit_credentials',
    'adfoin_get_kit_credentials',
    10,
    0
);
/*
...
add_action(
    'wp_ajax_adfoin_save_kit_credentials',
    'adfoin_save_kit_credentials',
    10,
    0
);
/*
 ...
 */
function adfoin_kit_modify_credentials(  $credentials, $platform  ) {
    if ( 'kit' == $platform && empty( $credentials ) ) {
        $api_key = get_option( 'adfoin_kit_api_key' );
        if ( $api_key ) {
...
function adfoin_kit_action_fields() {
    ...
    ?></option>
                    ...
                    <a href="<?php 
    echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=kit' );
