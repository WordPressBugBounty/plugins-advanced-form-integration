...
function adfoin_easysendy_settings_view(  $current_tab  ) {
    ...
    $instructions = sprintf( '<p>%s</p>', __( 'Click on the profile icon > APIs then create API Key. Copy the private key here.', 'advanced-form-integration' ) );
    ADFOIN_Account_Manager::render_settings_view(
        'easysendy',
        __( 'EasySendy', 'advanced-form-integration' ),
        $fields,
        $instructions
    );
}
...
add_action(
    'wp_ajax_adfoin_get_easysendy_credentials',
    'adfoin_get_easysendy_credentials',
    10,
    0
);
/*
...
add_action(
    'wp_ajax_adfoin_save_easysendy_credentials',
    'adfoin_save_easysendy_credentials',
    10,
    0
);
/*
...
add_filter(
    ...
    'adfoin_easysendy_modify_credentials',
    10,
    2
);
/*
 ...
 */
function adfoin_easysendy_modify_credentials(  $credentials, $platform  ) {
    if ( 'easysendy' == $platform && empty( $credentials ) ) {
        $api_key = get_option( 'adfoin_easysendy_api_key' );
        if ( $api_key ) {
...
function adfoin_easysendy_action_fields() {
    ...
    ?>
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php 
    esc_attr_e( 'EasySendy Account', 'advanced-form-integration' );
    ?>
                    </label>
                </td>
                <td>
    ...
    ?> </option>
                    ...
                    <a href="<?php 
    echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=easysendy' );
    ?>" target="_blank">
