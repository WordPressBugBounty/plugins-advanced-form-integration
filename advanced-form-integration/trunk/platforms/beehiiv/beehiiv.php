...
function adfoin_beehiiv_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'beehiiv', __( 'beehiiv', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_beehiiv_credentials', 'adfoin_get_beehiiv_credentials', 10, 0 );
/*
 * Get beehiiv credentials
 */
function adfoin_get_beehiiv_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'beehiiv' );
}

add_action( 'wp_ajax_adfoin_save_beehiiv_credentials', 'adfoin_save_beehiiv_credentials', 10, 0 );
/*
 * Save beehiiv credentials
 */
function adfoin_save_beehiiv_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'beehiiv', array( 'apiKey' ) );
}

/*
 * beehiiv Credentials List
 */
function adfoin_beehiiv_get_credentials_list() {
    $credentials = adfoin_read_credentials( 'beehiiv' );
    ...
    <script type="text/template" id="beehiiv-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'beehiiv Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=beehiiv' ); ?>" target="_blank">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Publication', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
...
function adfoin_beehiiv_send_data( $record, $posted_data ) {
    ...
    if( empty( $cred_id ) ) {
        $all_credentials = adfoin_read_credentials( 'beehiiv' );
    ...
    if( $task == 'subscribe' ) {
        ...
        if( $publication_id ) {
            $return = adfoin_beehiiv_request( "publications/{$publication_id}/subscriptions", 'POST', $subscriber_data, $record, $cred_id );
        }
    }
...
function adfoin_beehiiv_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {

    $credentials = adfoin_get_credentials_by_id( 'beehiiv', $cred_id );
