...
function adfoin_capsulecrm_settings_view( $current_tab ) {
    ...
    $fields = array(
        array( 
            'name' => 'apiToken', 
            'label' => __( 'API Authentication Token', 'advanced-form-integration' ), 
            'type' => 'text', 
            'required' => true,
            'mask' => true,
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Go to My Preferences > API Authentication Tokens > Generate New API Token', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'capsulecrm', __( 'Capsule CRM', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_capsulecrm_credentials', 'adfoin_get_capsulecrm_credentials', 10, 0 );
/*
 * Get Capsule CRM credentials
 */
function adfoin_get_capsulecrm_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'capsulecrm' );
}

add_action( 'wp_ajax_adfoin_save_capsulecrm_credentials', 'adfoin_save_capsulecrm_credentials', 10, 0 );
/*
 * Save Capsule CRM credentials
 */
function adfoin_save_capsulecrm_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'capsulecrm', array( 'apiToken' ) );
}

/*
 * Capsule CRM Credentials List
 */
function adfoin_capsulecrm_get_credentials_list() {
    $credentials = adfoin_read_credentials( 'capsulecrm' );
    ...
    <script type="text/template" id="capsulecrm-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_party'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Capsule CRM Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=capsulecrm' ); ?>" target="_blank">
 ...
 */
function adfoin_capsulecrm_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
    $credentials = adfoin_get_credentials_by_id( 'capsulecrm', $cred_id );
...
$teams    = adfoin_get_capsulecrm_teams( $cred_id );
...
function adfoin_get_capsulecrm_all_fields() {
    ...
    if( in_array( 'opportunity', $selected_objects ) ) {
        $milestones = adfoin_get_capsulecrm_opportunity_milestones( $cred_id );
...
function adfoin_capsulecrm_send_data( $record, $posted_data ) {
    ...
    if( empty( $cred_id ) ) {
        $all_credentials = adfoin_read_credentials( 'capsulecrm' );
    ...
    if( $task == "add_party" ) {
        ...
        if( isset( $org_data['name'] ) && $org_data['name'] ) {
            ...
            $org_id = adfoin_capsulecrm_party_exists( $org_holder['name'], $cred_id );
            ...
            $org_response = adfoin_capsulecrm_request( $endpoint, $method, array( 'party' => $org_holder ), $record, $cred_id );
        ...
        if( isset( $person_data['firstName'] ) && $person_data['firstName'] ) {
            ...
            $person_response = adfoin_capsulecrm_request( $endpoint, $method, array( 'party' => $person_holder ), $record, $cred_id );
        ...
        if( isset( $opportunity_data['name'] ) && $opportunity_data['name'] ) {
            ...
            $opportunity_response = adfoin_capsulecrm_request( $endpoint, $method, array( 'opportunity' => $opportunity_holder ), $record, $cred_id );
        ...
        if( isset( $case_data['name'] ) && $case_data['name'] ) {
            ...
            $case_response = adfoin_capsulecrm_request( $endpoint, $method, array( 'kase' => $case_holder ), $record, $cred_id );
