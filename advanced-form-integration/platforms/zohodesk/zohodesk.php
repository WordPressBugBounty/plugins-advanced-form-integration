...
class ADFOIN_ZohoDesk extends Advanced_Form_Integration_OAuth2 {
    ...
    protected $platform_slug = 'zohodesk';
    ...
    public function __construct() {
        ...
        add_filter( 'adfoin_settings_tabs', array( $this, 'adfoin_zohodesk_settings_tab' ), 10, 1 );
        add_action( 'adfoin_settings_view', array( $this, 'adfoin_zohodesk_settings_view' ), 10, 1 );
        add_action( 'adfoin_action_fields', array( $this, 'action_fields' ), 10, 1 );
        add_action( 'wp_ajax_adfoin_get_zohodesk_organizations', array( $this, 'get_organizations' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_get_zohodesk_owners', array( $this, 'get_owners' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_get_zohodesk_departments', array( $this, 'get_departments' ), 10, 0 );
        add_action( 'rest_api_init', array( $this, 'create_webhook_route' ) );
        add_action( 'wp_ajax_adfoin_get_zohodesk_fields', array( $this, 'get_fields' ) );
        add_action( 'wp_ajax_adfoin_get_zohodesk_credentials', array( $this, 'get_credentials' ), 10, 0 );
        add_filter( 'adfoin_get_credentials', array( $this, 'modify_credentials' ), 10, 2);
        add_action( 'wp_ajax_adfoin_save_zohodesk_credentials', array( $this, 'save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_zohodesk_connection', array( $this, 'test_connection' ), 10, 0 );
    }

    public function create_webhook_route() {
        register_rest_route( 'advancedformintegration', '/zohodesk',
            array(
                'methods'             => 'GET',
    ...
    public function get_webhook_data( $request ) {
        ...
        $context = self::consume_oauth_state( $state, 'zohodesk' );
    ...
    public function test_connection() {
        $this->run_test_connection_ajax( function () {
            return $this->zohodesk_request( 'organizations' );
        } );
    }
    ...
    */
    function save_credentials() {
        ...
        // Find existing record so we can preserve previously-saved values
        // when the edit form submits an empty client_id/client_secret.
        $existing = null;
        ...
        $auth_url = add_query_arg( array(
            ...
            'scope'         => $scope,
            'state'         => self::issue_oauth_state( 'zohodesk', $id )
        ), $auth_endpoint );
    ...
    public function adfoin_zohodesk_settings_view( $current_tab ) {
        ...
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';
        ADFOIN_OAuth_Manager::render_oauth_settings_view( 'zohodesk', 'Zoho Desk', $fields, $instructions, $config );
    }
            ...
            <table class='form-table'>
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    ...
                    <td>
                        ...
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=zohodesk' ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none;">
                ...
                <tr valign='top' class='alternate' v-if="action.task == 'subscribe'">
                    ...
                    <td>
                        ...
                        <div class="afi-spinner" v-bind:class="{'is-active': departmentLoading}"></div>
                    </td>
                </tr>
                ...
                <?php adfoin_pro_feature_notice( 'subscribe', 'Zoho Desk [PRO]', 'custom fields, tags and attachments' ); ?>
            </table>
        </script>
    ...
    public function zohodesk_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $org_id = '' ) {
    ...
    */
    public function get_departments() {
        ...
        $cred_id = isset( $_POST['credId'] ) ? sanitize_text_field( wp_unslash( $_POST['credId'] ) ) : '';
        $org_id = isset( $_POST['orgId'] ) ? sanitize_text_field( wp_unslash( $_POST['orgId'] ) ) : '';
        $this->set_credentials( $cred_id );
    ...
    function get_teams( $department_id, $org_id ) {
        $response = $this->zohodesk_request( "departments/{$department_id}/teams", 'GET', array(), array(), $org_id );
        $response_body = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    function get_agents( $org_id ) {
        $response = $this->zohodesk_request( "agents?status=ACTIVE&limit=200", 'GET', array(), array(), $org_id );
        $response_body = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    */
    function get_fields() {
        ...
        $cred_id = isset( $_POST['credId'] ) ? sanitize_text_field( wp_unslash( $_POST['credId'] ) ) : '';
        $org_id = isset( $_POST['orgId'] ) ? sanitize_text_field( wp_unslash( $_POST['orgId'] ) ) : '';
        $department_id = isset( $_POST['departmentId'] ) ? sanitize_text_field( wp_unslash( $_POST['departmentId'] ) ) : '';
        $this->set_credentials( $cred_id );
    ...
    public function search_record( $module, $search_key, $search_value ) {
        ...
        // URL-encode the value so emails / special characters don't break
        // the search query.
        $endpoint = sprintf(
            '%s/search?limit=1&%s=%s',
            rawurlencode( $module ),
            rawurlencode( $search_key ),
            rawurlencode( $search_value )
        );

        // Retry-on-empty handles Zoho Desk's brief index lag after writes.
        $delay    = (int) apply_filters( 'adfoin_zohodesk_search_delay', 2, $module );
        $attempts = (int) apply_filters( 'adfoin_zohodesk_search_attempts', 4, $module );
        if ( $delay   < 0 ) { $delay   = 0; }
    ...
    public function create_account( $account_data, $record, $org_id ) {
        ...
        $response = $this->zohodesk_request( 'accounts', 'POST', $account_data, $record, $org_id );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    public function update_account( $account_id, $account_data, $record, $org_id ) {
        

        $response = $this->zohodesk_request( 'accounts/' . $account_id, 'PUT', $account_data, $record, $org_id );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    public function create_contact( $contact_data, $record, $org_id ) {
        ...
        $response = $this->zohodesk_request( 'contacts', 'POST', $contact_data, $record, $org_id );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    public function update_contact( $contact_id, $contact_data, $record, $org_id ) {
        
        $response = $this->zohodesk_request( 'contacts/' . $contact_id, 'PUT', $contact_data, $record, $org_id );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    public function create_ticket( $ticket_data, $record, $org_id ) {
        ...
        $response = $this->zohodesk_request( 'tickets', 'POST', $ticket_data, $record, $org_id );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
...
add_action( 'adfoin_zohodesk_job_queue', 'adfoin_zohodesk_job_queue', 10, 1 );
...
function adfoin_zohodesk_send_data( $record, $posted_data ) {
    ...
    unset( $data['ownerId'], $data['orgId'], $data['departmentId'], $data['credId'] );
    ...
    if( $task == 'subscribe' ) {
        ...
        if( $account_data ) {
            ...
            if( $account_id ) {
                $zohodesk->update_account( $account_id, $account_data, $record, $org_id );
            } else{
        ...
        if( $contact_data ) {
            ...
            if( $contact_id ) {
                $zohodesk->update_contact( $contact_id, $contact_data, $record, $org_id );
            } else{
