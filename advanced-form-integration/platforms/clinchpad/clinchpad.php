...
function adfoin_clinchpad_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Please go to Settings > API to get API Key', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'clinchpad', __( 'ClinchPad', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_clinchpad_credentials', 'adfoin_get_clinchpad_credentials', 10, 0 );
/*
 * Get ClinchPad credentials
 */
function adfoin_get_clinchpad_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'clinchpad' );
}

add_action( 'wp_ajax_adfoin_save_clinchpad_credentials', 'adfoin_save_clinchpad_credentials', 10, 0 );
/*
 * Save ClinchPad credentials
 */
function adfoin_save_clinchpad_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'clinchpad', array( 'apiKey' ) );
}

/*
 * ClinchPad Credentials List
 */
function adfoin_clinchpad_get_credentials_list() {
    $credentials = adfoin_read_credentials( 'clinchpad' );
    ...
    <script type="text/template" id="clinchpad-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_contact'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'ClinchPad Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=clinchpad' ); ?>" target="_blank">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_contact'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Pipeline', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <div class="afi-spinner" v-bind:class="{'is-active': pipelineLoading}"></div>
                </td>
            </tr>
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_contact'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Stage', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                        ...
                        <div class="afi-spinner" v-bind:class="{'is-active': stageLoading}"></div>
                    </td>
                </tr>
...
function adfoin_clinchpad_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
    $credentials = adfoin_get_credentials_by_id( 'clinchpad', $cred_id );
...
function adfoin_create_lead( $lead, $record, $cred_id = '' ){
    $endpoint = 'leads';
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $lead, $record, $cred_id );
...
function adfoin_clinchpad_create_contact( $contact, $record, $cred_id = '' ){

    $endpoint = 'contacts';
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $contact, $record, $cred_id );
...
function adfoin_clinchpad_create_organization( $organization, $record, $cred_id = '' ){

    $endpoint = 'organizations';
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $organization, $record, $cred_id );
...
function adfoin_clinchpad_create_product( $product, $record, $cred_id = '' ){
    $endpoint = 'products';
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $product, $record, $cred_id );
...
function adfoin_clinchpad_create_note( $endpoint, $note_data, $record, $cred_id = '' ){
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $note_data, $record, $cred_id );
...
function adfoin_clinchpad_create_lead( $lead, $record, $cred_id = '' ){
    $endpoint = 'leads';
    $response = adfoin_clinchpad_request( $endpoint, 'POST', $lead, $record, $cred_id );
...
function adfoin_clinchpad_add_stage_to_lead( $lead_id, $stage, $record, $cred_id = '' ){
    $endpoint = "leads/{$lead_id}";

    $response = adfoin_clinchpad_request( $endpoint, 'PUT', $stage, $record, $cred_id );
...
function adfoin_clinchpad_send_data( $record, $posted_data ) {
    ...
    if( empty( $cred_id ) ) {
        $all_credentials = adfoin_read_credentials( 'clinchpad' );
    ...
    if( $task == 'add_contact' ) {
        ...
        if( $lead_name ){
            ...
            $lead_id = adfoin_clinchpad_create_lead( array_filter( $lead_data ), $record, $cred_id );
            ...
            if( $lead_id && $stage_id ){
                ...
                adfoin_clinchpad_add_stage_to_lead( $lead_id, $stage_data, $record, $cred_id );
            }
        ...
        if ( $organization ){
            ...
            $org_id = adfoin_clinchpad_create_organization( array_filter( $org_data ), $record, $cred_id );
        }
        ...
        if ( $name ){
            ...
            $contact_id = adfoin_clinchpad_create_contact( array_filter( $contact_data ), $record, $cred_id );

            if( $lead_id && $contact_id ){
                $endpoint = "leads/{$lead_id}/contacts/{$contact_id}";
                $cont_response = adfoin_clinchpad_request( $endpoint, 'PUT', array(), $record, $cred_id);
            }
        }
        ...
        if ( $product_name ){
            ...
            $product_id = adfoin_clinchpad_create_product( array_filter( $product ), $record, $cred_id );
            ...
            if( $lead_id && $product_id ){
                ...
                $prod_response = adfoin_clinchpad_request( $endpoint, 'PUT', $add_product, $record, $cred_id);
            }
        ...
        if ( $note ){
            ...
            if( $lead_id ) {
                ...
                $note_id = adfoin_clinchpad_create_note( $endpoint, $note_data, $record, $cred_id );
            }
        }
    }
