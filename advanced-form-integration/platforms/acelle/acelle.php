...
add_filter( 'adfoin_action_providers', 'adfoin_acelle_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_acelle_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_acelle_settings_view', 10, 1 );

function adfoin_acelle_settings_view( $current_tab ) {
    if( $current_tab != 'acelle' ) return;
    ...
    $arguments = wp_json_encode(
        [
            'platform' => $key,
            'fields' => [
                ['key' => 'apiEndpoint', 'label' => __( 'API Endpoint', 'advanced-form-integration' ), 'hidden' => false],
    ...
    $instructions = sprintf(
        ...
        __('Go to My Profile > Account > API Token.', 'advanced-form-integration'),
        __('Copy API Endpoint and API Token here.', 'advanced-form-integration')
    );
    ...
    <script type="text/template" id="acelle-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Acelle Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=advanced-form-integration-settings&tab=acelle' ) ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none;"><span class="dashicons dashicons-admin-settings" style="margin-top: 3px;"></span> <?php esc_html_e( 'Manage Accounts', 'advanced-form-integration' ); ?></a>
                </td>
            </tr>
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'Acelle Mail [PRO]', 'custom fields' ); ?>
 ...
 * Acelle API Request
 */
function adfoin_acelle_request( $endpoint, $method = 'GET', $data = [], $record = [], $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_acelle_credentials', 'adfoin_get_acelle_credentials', 10, 0 );
...
add_action( 'wp_ajax_adfoin_save_acelle_credentials', 'adfoin_save_acelle_credentials', 10, 0 );
...
add_action( 'wp_ajax_adfoin_get_acelle_list', 'adfoin_get_acelle_list', 10, 0 );
/*
...
add_action( 'adfoin_acelle_job_queue', 'adfoin_acelle_job_queue', 10, 1 );
...
function adfoin_acelle_send_data( $record, $posted_data ) {
    ...
    if ($task === 'subscribe') {
        ...
        adfoin_acelle_request( 'subscribers', 'POST', array_filter( $subscriber_data ), $record, $cred_id );
    }
