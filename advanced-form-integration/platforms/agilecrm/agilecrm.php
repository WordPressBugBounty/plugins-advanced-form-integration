...
add_filter( 'adfoin_action_providers', 'adfoin_agilecrm_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_agilecrm_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_agilecrm_settings_view', 10, 1 );
...
function adfoin_agilecrm_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'agilecrm', 'Agile CRM', $fields, $instructions );
}
        ...
        <div>
            <table class="form-table">
                <tr valign="top" v-if="action.task == 'add_contact'">
                    <th scope="row"><?php esc_html_e( 'Agile CRM Account', 'advanced-form-integration' ); ?></th>
                    <td>
                        <select name="fieldData[credId]" v-model="fielddata.credId" @change="fetchPipelines">
                            <option value=""> <?php _e( 'Select Account...', 'advanced-form-integration' ); ?> </option>
                            <option v-for="cred in credentialsList" :value="cred.id">{{ cred.title }}</option>
                        </select>
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=agilecrm' ); ?>" 
                           target="_blank" 
                ...
                <?php adfoin_pro_feature_notice( 'add_contact', 'Agile CRM [PRO]', 'tags & custom fields' ); ?>
            </table>
        </div>
    </script>
...
add_action( 'wp_ajax_adfoin_get_agilecrm_pipelines', 'adfoin_get_agilecrm_pipelines', 10, 0 );
...
function adfoin_get_agilecrm_pipelines() {
    ...
    if( !is_wp_error( $response ) ) {
        ...
        $deal_fields = array(
            array( 'key' => 'dealName', 'value' => 'Name [Deal]', 'description' => 'Required for Deal creation' ),
            array( 'key' => 'dealValue', 'value' => 'Value [Deal]', 'description' => 'Required for Deal creation' ),
...
add_action( 'adfoin_agilecrm_job_queue', 'adfoin_agilecrm_job_queue', 10, 1 );
...
function adfoin_agilecrm_send_data( $record, $posted_data ) {
    ...
    if( $task == 'add_contact' ) {
        ...
        if ( $address || $city || $state || $zip || $country ) {
            $body['properties'][] = array(
                'name'  => 'address',
                'value' => wp_json_encode( array_filter( array(
                    'address'     => $address,
                    'city'        => $city,
                    'state'       => $state,
                    'zip'         => $zip,
