...
class ADFOIN_Zohosheet extends Advanced_Form_Integration_OAuth2 {
    ...
    protected $platform_slug = 'zohosheet';
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_adfoin_get_zohosheet_credentials', array( $this, 'get_credentials' ), 10, 0 );
        add_filter( 'adfoin_get_credentials', array( $this, 'modify_credentials' ), 10, 2);
        add_action( 'wp_ajax_adfoin_save_zohosheet_credentials', array( $this, 'save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_zohosheet_connection', array( $this, 'test_connection' ), 10, 0 );
    ...
    public function get_webhook_data( $request ) {
        ...
        $context = self::consume_oauth_state( $state, 'zohosheet' );
        ...
        if ( $code ) {
            ...
            $zoho_credentials = adfoin_read_credentials( 'zohosheet' );
    ...
    public function test_connection() {
        $this->run_test_connection_ajax( function () {
            return $this->zohosheet_request( 'workbooks?method=workbook.list' );
        } );
    }
    ...
    function modify_credentials( $credentials, $platform ) {
        ...
        if ( 'zohosheet' == $platform && empty( $credentials ) ) {
    ...
    */
    function save_credentials() {
        ...
        $auth_url = add_query_arg( array(
            ...
            'scope'         => $scope,
            'state'         => self::issue_oauth_state( 'zohosheet', $id )
        ), $auth_endpoint );
    ...
    public function adfoin_zohosheet_settings_view( $current_tab ) {
        ...
        $instructions .= '<li>' . sprintf( __('Go to %s.', 'advanced-form-integration'), '<a target="_blank" rel="noopener noreferrer" href="https://api-console.zoho.com/">Zoho Sheet API Console</a>' ) . '</li>';
        ...
        $config = array(
            'show_status'  => true,
            'enable_test'  => true,
            'modal_title'  => __( 'Connect Zoho Sheet', 'advanced-form-integration' ),
            'submit_text'  => __( 'Save & Authorize', 'advanced-form-integration' ),
        );
        
        // Render using OAuth Manager
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';
        ADFOIN_OAuth_Manager::render_oauth_settings_view( 'zohosheet', 'Zoho Sheet', $fields, $instructions, $config );
    }
    ...
    public function get_credentials_list() {
        $html = '';
        $credentials = adfoin_read_credentials( 'zohosheet' );
    ...
    public function action_fields() {
        ...
        <script type="text/template" id="zohosheet-action-template">
            <table class="form-table">
                ...
                <?php adfoin_pro_feature_notice( 'add_row', 'Zoho Sheet [PRO]', 'updating existing rows' ); ?>
...
function adfoin_zohosheet_send_data( $record, $posted_data ) {
    ...
    if( $task == 'add_row' ) {
        ...
        $zohosheet = ADFOIN_Zohosheet::get_instance();
        $zohosheet->set_credentials( $cred_id );
        $holder    = array();
