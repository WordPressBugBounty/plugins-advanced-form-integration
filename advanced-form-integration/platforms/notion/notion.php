...
add_filter( 'adfoin_action_providers', 'adfoin_notion_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_notion_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_notion_settings_view', 10, 1 );
...
function adfoin_notion_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'notion', __( 'Notion', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_notion_credentials', 'adfoin_get_notion_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_notion_credentials', 'adfoin_save_notion_credentials', 10, 0 );
/*
...
// Legacy single-account import: surfaces old `adfoin_notion_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'notion', array(
            'apiToken' => 'adfoin_notion_api_token',
        ) );
    }
}, 20 );
    ...
    <script type="text/template" id="notion-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_item'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Notion Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=notion' ); ?>" target="_blank">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_item'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Database', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
            ...
            <?php adfoin_pro_feature_notice( 'add_item', 'Notion [PRO]', 'advanced features' ); ?>
 ...
 * Notion API Request
 */
function adfoin_notion_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_notion_databases', 'adfoin_get_notion_databases', 10, 0 );
/*
...
function adfoin_get_notion_databases() {
    ...
    while ( $has_more ) {
        ...
        $response = adfoin_notion_request( 'search', 'POST', $search_data, array(), $cred_id );
...
add_action( 'wp_ajax_adfoin_get_notion_fields', 'adfoin_get_notion_fields', 10, 0 );
/*
...
function adfoin_get_notion_fields() {
    ...
    $response = adfoin_notion_request( 'databases/' . $database_id, 'GET', array(), array(), $cred_id );
...
add_action( 'adfoin_notion_job_queue', 'adfoin_notion_job_queue', 10, 1 );
...
function adfoin_notion_send_data( $record, $posted_data ) {
    ...
    if ( $task == 'add_item' ) {
        ...
        $response = adfoin_notion_request( 'pages', 'POST', $page_data, $record, $cred_id );
    }
...
function adfoin_notion_format_property( $type, $value ) {
    ...
    switch ( $type ) {
        ...
        case 'checkbox':
            $bool_value = in_array( strtolower( $value ), array( 'true', 'yes', '1', 'on' ), true );
            return array(
                'checkbox' => $bool_value
            );
