Vue.component('ga4', {
    ...
    methods: {
        fetchCredentialsList: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_ga4_credentials', _nonce: adfoin.nonce }, function (response) {
        ...
        fetchFields: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_ga4_fields', _nonce: adfoin.nonce }, function (response) {
                that.fieldsLoading = false;
                if (response && response.success && Array.isArray(response.data)) {
                    that.fields = response.data.map(function (single) {
                        return { type: 'text', value: single.key, title: single.value, task: ['send_event'], required: !!single.required, description: single.description || '' };
                    });
                } else {
                    that.fields = [];
                }
