...
add_filter(
    ...
    'adfoin_convertkit_settings_tab',
    10,
    1
);
...
add_action(
    ...
    'adfoin_convertkit_settings_view',
    10,
    1
);
function adfoin_convertkit_settings_view(  $current_tab  ) {
    ...
    $instructions = sprintf( '<p>%s</p>', __( 'Please go to Account Settings > Advanced to get API Key', 'advanced-form-integration' ) );
    ADFOIN_Account_Manager::render_settings_view(
        'convertkit',
        __( 'ConvertKit', 'advanced-form-integration' ),
        $fields,
        $instructions
    );
}
...
add_action(
    'wp_ajax_adfoin_get_convertkit_credentials',
    'adfoin_get_convertkit_credentials',
    10,
    0
);
/*
...
add_action(
    'wp_ajax_adfoin_save_convertkit_credentials',
    'adfoin_save_convertkit_credentials',
    10,
    0
);
/*
...
add_filter(
    ...
    'adfoin_convertkit_modify_credentials',
    10,
    2
);
/*
 ...
 */
function adfoin_convertkit_modify_credentials(  $credentials, $platform  ) {
    if ( 'convertkit' == $platform && empty( $credentials ) ) {
        $api_key = get_option( 'adfoin_convertkit_api_key' );
        if ( $api_key ) {
...
add_action(
    'admin_post_adfoin_convertkit_save_api_key',
    'adfoin_save_convertkit_api_key',
    10,
    0
);
...
function adfoin_convertkit_action_fields() {
    ...
    ?>
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php 
    esc_attr_e( 'ConvertKit Account', 'advanced-form-integration' );
    ?>
                    </label>
                </td>
                <td>
    ...
    ?> </option>
                    ...
                    <a href="<?php 
    echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=convertkit' );
    ?>" target="_blank">
...
add_action(
    'wp_ajax_adfoin_get_convertkit_list',
    'adfoin_get_convertkit_list',
    10,
    0
);
/*
...
function adfoin_get_convertkit_list() {
    ...
    $data = adfoin_convertkit_request(
        'sequences',
        'GET',
        array(),
        array(),
...
add_action(
    'wp_ajax_adfoin_get_convertkit_forms',
    'adfoin_get_convertkit_forms',
    10,
    0
);
/*
...
function adfoin_convertkit_send_data(  $record, $posted_data  ) {
    ...
    if ( $task == 'subscribe' ) {
        ...
        if ( $sequence_id ) {
            ...
            $sequence_return = adfoin_convertkit_request(
                $sequence_endpoint,
                'POST',
                $data,
                $record,
        ...
        if ( $form_id ) {
            ...
            $form_return = adfoin_convertkit_request(
                $form_endpoint,
                'POST',
                $data,
                $record,
...
function adfoin_convertkit_request(
    $endpoint,
    $method = 'GET',
    $data = array(),
    $record = array(),
