...
function adfoin_convertkit_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Please go to Account Settings > Advanced to get API Key', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'convertkit', __( 'ConvertKit', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_convertkit_credentials', 'adfoin_get_convertkit_credentials', 10, 0 );
/*
 * Get ConvertKit credentials
 */
function adfoin_get_convertkit_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'convertkit' );
}

add_action( 'wp_ajax_adfoin_save_convertkit_credentials', 'adfoin_save_convertkit_credentials', 10, 0 );
/*
 * Save ConvertKit credentials
 */
function adfoin_save_convertkit_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'convertkit', array( 'apiKey' ) );
}

/*
 * ConvertKit Credentials List
 */
function adfoin_convertkit_get_credentials_list() {
    $credentials = adfoin_read_credentials( 'convertkit' );
...
// Legacy single-account import: surfaces old `adfoin_convertkit_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'convertkit', array(
            'apiKey' => 'adfoin_convertkit_api_key',
        ) );
    }
}, 20 );
...
function adfoin_save_convertkit_api_key() {
    ...
    $api_key = sanitize_text_field( wp_unslash( $_POST["adfoin_convertkit_api_key"] ) );
    ...
    <script type="text/template" id="convertkit-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'ConvertKit Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=convertkit' ); ?>" target="_blank">
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'ConvertKit [PRO]', 'custom fields and tags' ); ?>
...
function adfoin_get_convertkit_list() {
    ...
    $data = adfoin_convertkit_request( 'sequences', 'GET', array(), array(), $cred_id );
...
function adfoin_get_convertkit_forms() {
    ...
    $cred_id = isset( $_POST['credId'] ) ? sanitize_text_field( wp_unslash( $_POST['credId'] ) ) : '';
    $data = adfoin_convertkit_request( 'forms', 'GET', array(), array(), $cred_id );
...
function adfoin_convertkit_send_data( $record, $posted_data ) {
    ...
    if( empty( $cred_id ) ) {
        $all_credentials = adfoin_read_credentials( 'convertkit' );
    ...
    if( $task == 'subscribe' ) {
        ...
        if( $sequence_id ) {
            ...
            $sequence_return   = adfoin_convertkit_request( $sequence_endpoint, 'POST', $data, $record, $cred_id );
        }

        if( $form_id ) {
            $form_endpoint = "forms/{$form_id}/subscribe";
            $form_return   = adfoin_convertkit_request( $form_endpoint, 'POST', $data, $record, $cred_id );
        }
    }
...
function adfoin_convertkit_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
    $credentials = adfoin_get_credentials_by_id( 'convertkit', $cred_id );
