...
add_filter( 'adfoin_action_providers', 'adfoin_autopilotnew_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_autopilotnew_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_autopilotnew_settings_view', 10, 1 );
...
function adfoin_autopilotnew_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'autopilotnew', 'Ortto', $fields, $instructions );
}
...
function adfoin_autopilotnew_get_credentials_list_ajax() {
    ...
    $fields = array(
        array( 'name' => 'data_center', 'mask' => false ),
...
add_action( 'admin_post_adfoin_autopilotnew_save_api_key', 'adfoin_save_autopilotnew_api_key', 10, 0 );
...
function adfoin_save_autopilotnew_api_key() {
    ...
    $api_key     = isset( $_POST['adfoin_autopilotnew_api_key'] ) ? sanitize_text_field( wp_unslash( $_POST['adfoin_autopilotnew_api_key'] ) ) : '';
    $data_center = isset( $_POST['adfoin_autopilotnew_data_center'] ) ? sanitize_text_field( wp_unslash( $_POST['adfoin_autopilotnew_data_center'] ) ) : '';
    ...
    <script type="text/template" id="autopilotnew-action-template">
        <table class="form-table">
            <tr valign="top" v-if="action.task == 'subscribe'">
                <th scope="row"><?php esc_html_e( 'Ortto Account', 'advanced-form-integration' ); ?></th>
                <td>
                    <select name="fieldData[credId]" v-model="fielddata.credId">
                        <option value=""> <?php _e( 'Select Account...', 'advanced-form-integration' ); ?> </option>
                        <option v-for="cred in credentialsList" :value="cred.id">{{ cred.title }}</option>
                    </select>
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=autopilotnew' ); ?>" 
                       target="_blank" 
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'Ortto [PRO]', 'custom fields' ); ?>
        </table>
    </script>
    <?php
}
...
function adfoin_autopilotnew_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        if( $first_name ) { $data['people'][0]['fields']['str::first'] = $first_name; }
        if( $last_name ) { $data['people'][0]['fields']['str::last'] = $last_name; }

        $return = adfoin_autopilotnew_request( 'person/merge', 'POST', $data, $record, $cred_id );
    }

    return;
}

add_action( 'adfoin_autopilotnew_job_queue', 'adfoin_autopilotnew_job_queue', 10, 1 );
...
// Ortto API Call
function adfoin_autopilotnew_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
