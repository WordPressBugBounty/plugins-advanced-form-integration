...
add_filter( 'adfoin_action_providers', 'adfoin_brevo_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_brevo_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_brevo_settings_view', 10, 1 );
...
function adfoin_brevo_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'brevo', __( 'Brevo', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_brevo_credentials', 'adfoin_get_brevo_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_brevo_credentials', 'adfoin_save_brevo_credentials', 10, 0 );
/*
    ...
    <script type="text/template" id="brevo-action-template">
        ...
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Brevo Account', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                        ...
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=brevo' ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none;">
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Brevo List', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                ...
                <?php adfoin_pro_feature_notice( 'subscribe', 'Brevo [PRO]', 'custom fields' ); ?>
...
add_action( 'wp_ajax_adfoin_get_brevo_list', 'adfoin_get_brevo_list', 10, 0 );
/*
...
function adfoin_get_brevo_list() {
    ...
    while( $has_value ) {
        ...
        $response = adfoin_brevo_request( $endpoint, 'GET', [], [], $cred_id );
...
add_action( 'adfoin_brevo_job_queue', 'adfoin_brevo_job_queue', 10, 1 );
...
function adfoin_brevo_send_data( $record, $posted_data ) {
    ...
    if ( $task == 'subscribe' ) {
        ...
        $response = adfoin_brevo_request( 'contacts', 'POST', $data, $record, $cred_id );
    }
