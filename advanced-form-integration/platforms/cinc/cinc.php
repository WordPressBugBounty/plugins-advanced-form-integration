...
/**
 * CINC — Create Lead via POST /v2/site/leads (an upsert keyed off
 * lead.username or lead.info.contact.email).
 *
 * Multi-account OAuth via AFI's OAuth Manager popup flow.
 * Auth: Bearer {access_token}; tokens refreshed automatically.
 *
 * The real Lead schema nests contact info under `info.contact` and
 * represents "lead type" as two booleans (`is_buyer`/`is_seller`), not a
 * free-text field — and has no property-search-criteria fields at all.
 * None of this matches the flat `leads` resource + `searchCriteria` object
 * the previous version of this file assumed, nor its static Bearer-API-key
 * auth model (CINC is OAuth2 only).
 *
 * @link https://public.cincapi.com/v2/docs/
 */

class ADFOIN_Cinc extends Advanced_Form_Integration_OAuth2 {

    protected $platform_slug = 'cinc';

    const authorization_endpoint = 'https://public.cincapi.com/auth/login';
    const token_endpoint         = 'https://authv2.cincapi.com/integrator/token';
    const refresh_token_endpoint = 'https://authv2.cincapi.com/integrator/token';
    const api_base_url           = 'https://public.cincapi.com/v2/';
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_adfoin_get_cinc_credentials', array( $this, 'ajax_get_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_save_cinc_credentials', array( $this, 'ajax_save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_cinc_connection', array( $this, 'ajax_test_connection' ), 10, 0 );
    }

    public function register_actions( $actions ) {
        $actions['cinc'] = array(
            'title' => __( 'CINC', 'advanced-form-integration' ),
            'tasks' => array(
    ...
    public function register_settings_tab( $providers ) {
        $providers['cinc'] = __( 'CINC', 'advanced-form-integration' );
        return $providers;
    }
    ...
    public function settings_view( $current_tab ) {
        ...
        $instructions .= '<li>' . __( 'CINC enables API access through their integration team — email integrations@cincpro.com to request a Client ID/Secret and register a Redirect URI.', 'advanced-form-integration' ) . '</li>';
        ...
        $config = array(
            'show_status' => true,
            'enable_test' => true,
            'modal_title' => __( 'Connect CINC', 'advanced-form-integration' ),
        ...
        ADFOIN_OAuth_Manager::render_oauth_settings_view(
            'cinc',
            __( 'CINC', 'advanced-form-integration' ),
            $fields,
            $instructions,
            $config
        );
            ...
            <table class="form-table">
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'create_lead'">
                    ...
                    <td>
                        ...
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=advanced-form-integration-settings&tab=cinc' ) ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none; vertical-align: middle;">
                            <span class="dashicons dashicons-admin-settings" style="margin-top: 3px;"></span> <?php esc_html_e( 'Manage Accounts', 'advanced-form-integration' ); ?>
                        </a>
                    </td>
                </tr>
                <editable-field v-for="field in fields" v-bind:key="field.value" v-bind:field="field" v-bind:trigger="trigger" v-bind:action="action" v-bind:fielddata="fielddata"></editable-field>
                <?php adfoin_pro_feature_notice( 'create_lead', 'CINC [PRO]', 'tags and custom fields' ); ?>
            </table>
        </script>
        <?php
    }
    ...
    public function create_webhook_route() {
        register_rest_route( 'advancedformintegration', '/cinc', array(
            'methods'             => 'GET',
    ...
    /**
     * REST callback hit by CINC with ?code=…&state=…. Resolves the saved
    ...
    public function get_webhook_data( $request ) {
        ...
        $context = self::consume_oauth_state( $state, 'cinc' );
        ...
        $found = false;
        foreach ( adfoin_read_credentials( 'cinc' ) as $entry ) {
            if ( ( $entry['id'] ?? '' ) === $cred_id ) {
    ...
    public function ajax_save_credentials() {
        ...
        $auth_url = add_query_arg( array(
            ...
            'client_id'     => $client_id,
            'redirect_uri'  => $this->get_redirect_uri(),
            'scope'         => 'api:read api:create',
            'state'         => self::issue_oauth_state( 'cinc', $id ),
    ...
    public function ajax_test_connection() {
        $this->run_test_connection_ajax( function () {
            return $this->cinc_request( 'site', 'GET' );
        } );
    }

    protected function get_redirect_uri() {
        return site_url( '/wp-json/advancedformintegration/cinc' );
    }
    ...
    public function get_credentials_by_id( $cred_id ) {
        ...
        foreach ( adfoin_read_credentials( 'cinc' ) as $single ) {
    ...
    public function cinc_request( $endpoint, $method = 'GET', $data = array(), $record = array() ) {
...
ADFOIN_Cinc::get_instance();

add_action( 'wp_ajax_adfoin_get_cinc_fields', 'adfoin_get_cinc_fields' );
function adfoin_get_cinc_fields() {
    adfoin_verify_nonce();
    // leadType is not a free-text field in the real schema — it's two
    // booleans (is_buyer/is_seller) — and there are no property
    // search-criteria fields on Lead at all, so priceMin/priceMax/beds/
    // baths are dropped rather than silently discarded after mapping.
    $fields = array(
        array( 'key' => 'firstName', 'value' => 'First Name', 'description' => '' ),
        array( 'key' => 'lastName',  'value' => 'Last Name',  'description' => '' ),
        array( 'key' => 'email',     'value' => 'Email',      'description' => '' ),
        array( 'key' => 'phone',     'value' => 'Phone',      'description' => '' ),
        array( 'key' => 'isBuyer',   'value' => 'Is Buyer (true/false)',  'description' => '' ),
        array( 'key' => 'isSeller',  'value' => 'Is Seller (true/false)', 'description' => '' ),
...
/**
 * Builds the /v2/site/leads upsert body. Shared with the Pro tier.
 * @link https://public.cincapi.com/v2/docs/examples/leads/upsert_lead.html
 */
function adfoin_cinc_build_lead( $fields ) {
    $contact = array();
    foreach ( array( 'firstName' => 'first_name', 'lastName' => 'last_name', 'email' => 'email' ) as $local => $remote ) {
        if ( ! empty( $fields[ $local ] ) ) $contact[ $remote ] = $fields[ $local ];
    }
    if ( ! empty( $fields['phone'] ) ) $contact['phone_numbers'] = array( 'cell_phone' => $fields['phone'] );

    $address = array();
    foreach ( array( 'street' => 'street', 'city' => 'city', 'state' => 'state', 'zip' => 'postal_or_zip' ) as $local => $remote ) {
        if ( ! empty( $fields[ $local ] ) ) $address[ $remote ] = $fields[ $local ];
    }
    if ( $address ) $contact['mailing_address'] = $address;

    $info = array( 'contact' => $contact );
    if ( isset( $fields['isBuyer'] ) && $fields['isBuyer'] !== '' )   $info['is_buyer']  = filter_var( $fields['isBuyer'], FILTER_VALIDATE_BOOLEAN );
    if ( isset( $fields['isSeller'] ) && $fields['isSeller'] !== '' ) $info['is_seller'] = filter_var( $fields['isSeller'], FILTER_VALIDATE_BOOLEAN );
    if ( ! empty( $fields['source'] ) ) $info['source'] = $fields['source'];

    return array( 'lead' => array( 'info' => $info ) );
}
...
function adfoin_cinc_send_data( $record, $posted_data ) {
    ...
    if ( empty( $fields['email'] ) ) return; // Required — the upsert keys off lead.info.contact.email.

    $cinc = ADFOIN_Cinc::get_instance();
    if ( ! $cinc->set_credentials( $cred_id ) ) return;

    $cinc->cinc_request( 'site/leads', 'POST', adfoin_cinc_build_lead( $fields ), $record );
}
