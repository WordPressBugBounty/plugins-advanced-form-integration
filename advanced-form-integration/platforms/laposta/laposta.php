...
// add_action('wp_ajax_adfoin_get_laposta_list_fields', 'adfoin_get_laposta_list_fields');
// function adfoin_get_laposta_list_fields() {
//     if (!adfoin_verify_nonce()) return;

//     $cred_id = sanitize_text_field($_POST['credId']);
//     $list_id = sanitize_text_field($_POST['listId']);
//     $fieldsRes = adfoin_laposta_request("field?list_id={$list_id}", 'GET', [], [], $cred_id);

//     if (is_wp_error($fieldsRes)) wp_send_json_error();

//     $body = json_decode(wp_remote_retrieve_body($fieldsRes), true);
//     $fields = [];

//     if (isset($body['response']['items'])) {
