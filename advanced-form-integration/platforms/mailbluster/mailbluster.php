...
add_filter( 'adfoin_action_providers', 'adfoin_mailbluster_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_mailbluster_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_mailbluster_settings_view', 10, 1 );
...
function adfoin_mailbluster_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Go to Settings > API Keys and create new API Key', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'mailbluster', __( 'MailBluster', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_mailbluster_credentials', 'adfoin_get_mailbluster_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_mailbluster_credentials', 'adfoin_save_mailbluster_credentials', 10, 0 );
/*
...
// Legacy single-account import: surfaces old `adfoin_mailbluster_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'mailbluster', array(
            'apiKey' => 'adfoin_mailbluster_api_token',
        ) );
    }
}, 20 );
        ...
        <table class="form-table">
            <tr valign="top" v-if="action.task == 'add_contact'">
                ...
                <td scope="row">
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=mailbluster' ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none;"><span class="dashicons dashicons-admin-settings" style="margin-top: 3px;"></span> <?php esc_html_e( 'Manage Accounts', 'advanced-form-integration' ); ?></a>
            ...
            <?php adfoin_pro_feature_notice( 'add_contact', 'MailBluster [PRO]', 'tags and custom fields' ); ?>
 ...
 */
function adfoin_mailbluster_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
function adfoin_mailbluster_lead_exists( $hash, $cred_id = '' ) {
    ...
    $return = adfoin_mailbluster_request( 'leads/' . $hash, 'GET', array(), array(), $cred_id );
...
add_action( 'adfoin_mailbluster_job_queue', 'adfoin_mailbluster_job_queue', 10, 1 );
...
function adfoin_mailbluster_send_data( $record, $posted_data ) {
    ...
    if( $task == 'add_contact' ) {
        ...
        $basic_fields = array( 'email', 'firstName', 'lastName', 'fullName', 'timezone', 'ipAddress' );
        ...
        if( $body ) {
            ...
            if( adfoin_mailbluster_lead_exists( $email_hash, $cred_id ) ) {
                adfoin_mailbluster_request( 'leads/' . $email_hash, 'PUT', $body, $record, $cred_id );
            } else{
                adfoin_mailbluster_request( 'leads', 'POST', $body, $record, $cred_id );
            }
        }
    }
