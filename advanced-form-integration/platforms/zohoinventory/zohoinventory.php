...
class ADFOIN_ZohoInventory extends Advanced_Form_Integration_OAuth2 {

    protected $platform_slug = 'zohoinventory';
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_adfoin_get_zohoinventory_credentials', array( $this, 'get_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_save_zohoinventory_credentials', array( $this, 'save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_zohoinventory_connection', array( $this, 'test_connection' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_get_zohoinventory_organizations', array( $this, 'ajax_get_organizations' ), 10, 0 );
    }

    public function register_actions( $actions ) {
        $actions['zohoinventory'] = array(
            'title' => __( 'Zoho Inventory', 'advanced-form-integration' ),
            'tasks' => array(
                ...
                'create_salesorder'  => __( 'Sales Order', 'advanced-form-integration' ),
                'create_invoice'     => __( 'Invoice', 'advanced-form-integration' ),
                'create_package'     => __( 'Package', 'advanced-form-integration' ),
            ),
        );
        return $actions;
    }
    ...
    public function register_settings_tab( $providers ) {
        $providers['zohoinventory'] = __( 'Zoho Inventory', 'advanced-form-integration' );
        return $providers;
    }

    public function settings_view( $current_tab ) {
        if ( 'zohoinventory' !== $current_tab ) {
            return;
        }
        ...
        $config = array(
            'show_status' => true,
            'enable_test' => true,
            'modal_title' => __( 'Connect Zoho Inventory', 'advanced-form-integration' ),
            'submit_text' => __( 'Save & Authorize', 'advanced-form-integration' ),
        );

        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';
        ADFOIN_OAuth_Manager::render_oauth_settings_view( 'zohoinventory', 'Zoho Inventory', $fields, $instructions, $config );
    }
    ...
    public function get_webhook_data( $request ) {
        ...
        $context = self::consume_oauth_state( $raw_state, 'zohoinventory' );
        $cred_id = $context ? $context['cred_id'] : '';

        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';

        if ( ! $cred_id ) {
            ADFOIN_OAuth_Manager::handle_callback_close_popup(
                false,
        ...
        $credentials   = adfoin_read_credentials( 'zohoinventory' );
    ...
    public function auth_redirect() {
        // Kept for back-compat with users whose Zoho app still points to the
        // admin-side redirect. New installs use the REST callback above.
        $action = isset( $_GET['action'] ) ? sanitize_text_field( wp_unslash( $_GET['action'] ) ) : '';
        if ( 'adfoin_zohoinventory_auth_redirect' !== $action ) {
            return;
        }

        $code  = isset( $_GET['code'] ) ? sanitize_text_field( wp_unslash( $_GET['code'] ) ) : '';
        $state = isset( $_GET['state'] ) ? sanitize_text_field( wp_unslash( $_GET['state'] ) ) : '';
        $context = self::consume_oauth_state( $state, 'zohoinventory' );
        $cred_id = $context ? $context['cred_id'] : '';
    ...
    protected function get_redirect_uri() {
        return site_url( '/wp-json/advancedformintegration/zohoinventory' );
    }
    ...
    protected function get_apis_base_url() {
        $map = array(
            ...
            'com.cn' => 'https://www.zohoapis.com.cn/inventory/v1/',
    ...
    public function test_connection() {
        $this->run_test_connection_ajax( function () {
            return $this->zohoinventory_request( 'organizations' );
        } );
    }
    ...
    public function save_credentials() {
        ...
        $auth_url = add_query_arg(
            array(
                ...
                'scope'         => $scope,
                'state'         => self::issue_oauth_state( 'zohoinventory', $id ),
            ),
            $auth_endpoint
        );
    ...
    public function get_credentials_by_id( $cred_id ) {
        $all = adfoin_read_credentials( 'zohoinventory' );
    ...
    public function action_fields() {
        ?>
        <script type="text/template" id="zohoinventory-action-template">
            <table class="form-table">
                <tr>
                    <th scope="row"><?php esc_attr_e( 'Zoho Inventory Account', 'advanced-form-integration' ); ?></th>
                    <td>
                        ...
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=zohoinventory' ); ?>" target="_blank">
                ...
                <tr v-if="['create_salesorder','create_invoice','create_package'].indexOf(action.task) !== -1">
                    ...
                    <td>
                        ...
                        <p class="description"><?php esc_html_e( 'Choose "WooCommerce Order Items" when this integration is triggered by a WooCommerce order — line items will be built automatically from the order.', 'advanced-form-integration' ); ?></p>
                    </td>
                </tr>
    ...
    public function ajax_get_organizations() {
        ...
        $response = $this->zohoinventory_request( 'organizations', 'GET' );
    ...
    public function zohoinventory_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $organization_id = '' ) {
        ...
        if ( $organization_id ) {
            ...
            $args['headers']['X-com-zoho-inventory-organizationid'] = $organization_id;
        }
    ...
    /**
     * Build a Zoho Inventory contact payload from the action field data and
     * the trigger's posted_data. Handles flat customer fields plus optional
    ...
    /**
     * Build a Zoho address payload from $field_data using a prefix
     * ('billing' or 'shipping'). All keys optional.
     */
    ...
    /**
     * Search a customer by email, then by contact_name. Returns the first
     * matching record, or empty array if not found.
     */
    public function find_customer( $customer_payload, $org_id, $record = array() ) {
        ...
        if ( $email ) {
            $search_attempts[] = array( 'email_contains' => $email );
        }
        if ( $name ) {
            $search_attempts[] = array( 'contact_name_contains' => $name );
        }

        foreach ( $search_attempts as $params ) {
            $endpoint = add_query_arg( $params, 'contacts' );
            $response = $this->zohoinventory_request( $endpoint, 'GET', array(), $record, $org_id );
            if ( ! is_wp_error( $response ) ) {
                $body = json_decode( wp_remote_retrieve_body( $response ), true );
    ...
    public function find_or_create_customer( $customer_payload, $org_id, $record = array() ) {
        ...
        $create = $this->zohoinventory_request( 'contacts', 'POST', $customer_payload, $record, $org_id );
    ...
    /**
     * Find an item by SKU first (more reliable), then by name. If neither
     * matches, create one and return the created item.
     */
    public function find_or_create_item( $item_payload, $org_id, $record = array() ) {
        ...
        $search_attempts = array();
        if ( $sku ) {
            $search_attempts[] = array( 'sku' => $sku );
        }
        if ( $name ) {
            $search_attempts[] = array( 'name' => $name );
        }
        ...
        if ( empty( $item_payload['item_type'] ) ) {
            $item_payload['item_type'] = 'inventory';
        }
        if ( empty( $item_payload['product_type'] ) ) {
            $item_payload['product_type'] = 'goods';
        }

        $create = $this->zohoinventory_request( 'items', 'POST', $item_payload, $record, $org_id );
        if ( is_wp_error( $create ) ) {
            return array();
        }
        $body = json_decode( wp_remote_retrieve_body( $create ), true );
    ...
    /**
     * Build Zoho line_items array from raw entries (one per item).
     * Each entry is an associative array carrying at least `item_name`
     * (or `name`) and `quantity`/`rate`. Optional: `sku`, `description`,
     * `tax_id`.
     */
    public function build_line_items( $entries, $org_id, $record = array() ) {
        ...
        foreach ( $entries as $entry ) {
            ...
            if ( ! empty( $entry['sku'] ) ) {
                $item_payload['sku'] = $entry['sku'];
            }
            if ( isset( $entry['rate'] ) ) {
                $item_payload['rate'] = (float) $entry['rate'];
            }
    ...
    /**
     * WooCommerce trigger pushes line-item fields as parallel arrays:
     *   $posted_data['items_id']       = [id1, id2, …]
     *   $posted_data['items_name']     = [name1, name2, …]
     *   $posted_data['items_sku']      = [sku1, sku2, …]
     *   $posted_data['items_price']    = [price1, price2, …]
     *   $posted_data['items_quantity'] = [qty1, qty2, …]
     *
     * Convert that into an entries array that `build_line_items()` understands.
     */
    public function build_wc_line_item_entries( $posted_data ) {
        ...
        // Single-item orders may arrive as scalars; normalize.
...
$adfoin_zohoinventory = ADFOIN_ZohoInventory::get_instance();

function adfoin_zohoinventory_job_queue( $data ) {
    if ( ( $data['action_provider'] ?? '' ) !== 'zohoinventory' ) {
        return;
    }
    adfoin_zohoinventory_send_data( $data['record'], $data['posted_data'] );
}
add_action( 'adfoin_job_queue', 'adfoin_zohoinventory_job_queue', 10, 1 );

function adfoin_zohoinventory_send_data( $record, $posted_data ) {
    ...
    $inv = ADFOIN_ZohoInventory::get_instance();
    ...
    switch ( $task ) {
        case 'upsert_customer':
            ...
            $inv->find_or_create_customer( $customer_payload, $org_id, $record );
            break;
        ...
        case 'upsert_item':
            ...
            $item_type = adfoin_get_parsed_values( $field_data['item_type'] ?? '', $posted_data );
            if ( $item_type ) {
        ...
        case 'create_invoice':
        case 'create_package':
            adfoin_zohoinventory_handle_document( $inv, $task, $field_data, $posted_data, $org_id, $record );
            break;
    }
}

function adfoin_zohoinventory_handle_document( $inv, $task, $field_data, $posted_data, $org_id, $record ) {
    $customer_payload = $inv->build_customer_payload( $field_data, $posted_data );
    if ( empty( $customer_payload['contact_name'] ) ) {
        return;
    }
    ...
    // Line items: either auto-built from a WooCommerce order, or from a
    // user-provided JSON payload (mappable to a form field).
    ...
    // Discount / shipping charges (numeric).
    foreach ( array( 'discount', 'shipping_charge', 'adjustment' ) as $num_key ) {
        $raw = adfoin_get_parsed_values( $field_data[ $num_key ] ?? '', $posted_data );
        if ( '' !== $raw && null !== $raw ) {
            $payload[ $num_key ] = (float) $raw;
        }
    }
    ...
    switch ( $task ) {
        ...
        case 'create_invoice':
            ...
            break;
        case 'create_package':
            // Packages require an existing salesorder_id rather than a customer_id.
            $salesorder_id = adfoin_get_parsed_values( $field_data['salesorder_id'] ?? '', $posted_data );
            if ( ! $salesorder_id ) {
                return;
            }
            $endpoint = add_query_arg( array( 'salesorder_id' => $salesorder_id ), 'packages' );
            unset( $payload['customer_id'] );
            break;
    }

    if ( $endpoint ) {
        $inv->zohoinventory_request( $endpoint, 'POST', $payload, $record, $org_id );
    }
}
