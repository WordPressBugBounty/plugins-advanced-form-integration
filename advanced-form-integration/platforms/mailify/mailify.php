...
add_filter( 'adfoin_action_providers', 'adfoin_mailify_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_mailify_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_mailify_settings_view', 10, 1 );
...
function adfoin_mailify_settings_view( $current_tab ) {
    ...
    ADFOIN_Account_Manager::render_settings_view( 'mailify', __( 'Mailify', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_mailify_credentials', 'adfoin_get_mailify_credentials', 10, 0 );
...
add_action( 'wp_ajax_adfoin_save_mailify_credentials', 'adfoin_save_mailify_credentials', 10, 0 );
...
// Legacy single-account import: surfaces old `adfoin_mailify_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'mailify', array(
            'accountNumber' => 'adfoin_mailify_account_number',
            'apiKey' => 'adfoin_mailify_key',
        ) );
    }
}, 20 );
add_action( 'adfoin_add_js_fields', 'adfoin_mailify_js_fields', 10, 1 );
...
function adfoin_mailify_action_fields() {
    ...
    <script type="text/template" id="mailify-action-template">
        <table class="form-table">
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'Mailify [PRO]', 'all fields' ); ?>
        </table>
    </script>
    <?php
}

add_action( 'wp_ajax_adfoin_get_mailify_list', 'adfoin_get_mailify_list', 10, 0 );
/*
...
function adfoin_get_mailify_list() {
    ...
    $data = adfoin_mailify_request( 'lists', 'GET', array(), array(), $cred_id );
...
function adfoin_mailify_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        $return = adfoin_mailify_request( "lists/{$list_id}/contacts", 'POST', $data, $record, $cred_id );
    }
 ...
 */
function adfoin_mailify_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
