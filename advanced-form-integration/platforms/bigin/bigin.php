...
class ADFOIN_Bigin extends Advanced_Form_Integration_OAuth2 {
    ...
    private function __construct() {
        ...
        add_filter( 'adfoin_settings_tabs', array( $this, 'adfoin_bigin_settings_tab' ), 10, 1 );
        add_action( 'adfoin_settings_view', array( $this, 'adfoin_bigin_settings_view' ), 10, 1 );
        add_action( 'adfoin_action_fields', array( $this, 'action_fields' ), 10, 1 );
        add_action( 'wp_ajax_adfoin_get_bigin_users', array( $this, 'get_users' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_get_bigin_modules', array( $this, 'get_modules' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_get_bigin_module_fields', array( $this, 'get_fields' ) );
        add_action( 'wp_ajax_adfoin_get_bigin_credentials', array( $this, 'get_credentials' ), 10, 0 );
        add_filter( 'adfoin_get_credentials', array( $this, 'modify_credentials' ), 10, 2 );
        add_action( 'wp_ajax_adfoin_save_bigin_credentials', array( $this, 'save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_bigin_connection', array( $this, 'test_connection' ), 10, 0 );
    }

    /**
     * Test a credential by hitting a minimal Bigin endpoint. Used by the
     * "Test Connection" button on the accounts table — gives the admin a
     * single-click signal that OAuth still works, before a real form is
     * submitted.
     */
    ...
    public function create_webhook_route() {
        register_rest_route( 'advancedformintegration', '/bigin',
            array(
                'methods'             => 'GET',
    ...
    public function get_webhook_data( $request ) {
        ...
        // OAuth Manager flow: a state token is round-tripped. Validate it,
        // exchange the code for tokens, and close the popup.
        if ( $raw_state ) {
            $context = self::consume_oauth_state( $raw_state, 'bigin' );
            $cred_id = $context ? $context['cred_id'] : '';

            require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';

            if ( ! $cred_id ) {
                // State was provided but couldn't be validated (expired,
                // already used, or user mismatch). Surface the failure in
                // the popup instead of falling through to the legacy
                // admin redirect — the state transient has already been
                // consumed and can't be recovered there.
        ...
        // Legacy flow - redirect to admin (no state provided at all)
    ...
    public function adfoin_bigin_settings_view( $current_tab ) {
        ...
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';
        ADFOIN_OAuth_Manager::render_oauth_settings_view( 'bigin', 'Bigin', $fields, $instructions, $config );
    }

    /**
     * Get credentials via AJAX
     *
     * safe_credentials_list() strips client_id/client_secret. We merge them
     * back so the OAuth Manager table can display them (masked client-side)
     * and the edit form can pre-fill them.
     */
    public function get_credentials() {
        ...
        $raw  = adfoin_read_credentials( 'bigin' );

        if ( is_array( $raw ) ) {
            $by_id = array();
            foreach ( $raw as $cred ) {
                if ( is_array( $cred ) && isset( $cred['id'] ) ) {
                    $by_id[ (string) $cred['id'] ] = $cred;
                }
            }
            foreach ( $list as &$row ) {
                $id = isset( $row['id'] ) ? (string) $row['id'] : '';
                if ( $id && isset( $by_id[ $id ] ) ) {
                    // client_id is OK to ship to the browser — it appears in
                    // OAuth auth URLs anyway. The table masks it client-side,
                    // and the edit form needs it to pre-fill.
                    if ( isset( $by_id[ $id ]['client_id'] ) ) {
                        $row['client_id'] = $by_id[ $id ]['client_id'];
                    }
                    // client_secret stays out of the response entirely. The
                    // edit form shows it as blank with a "leave blank to keep
                    // current" placeholder; save_credentials preserves the
                    // stored value when an empty client_secret is submitted.
    ...
    public function save_credentials() {
        ...
        // (can happen on legacy rows or stale form state, and is the normal
        // case for client_secret which we never ship to the browser) must
        // not wipe out the previously-saved values.
        if ( $existing ) {
            ...
            // New account creation: both fields are mandatory.
        ...
        if ( $existing ) {
            ...
            if ( isset( $existing['client_id'] ) && $existing['client_id'] == $client_id &&
                 isset( $existing['client_secret'] ) && $existing['client_secret'] == $client_secret &&
                 isset( $existing['data_center'] ) && $existing['data_center'] == $data_center ) {
                $new_data['access_token']  = isset( $existing['access_token'] ) ? $existing['access_token'] : '';
                $new_data['refresh_token'] = isset( $existing['refresh_token'] ) ? $existing['refresh_token'] : '';
            }
            foreach ( $credentials as &$cred ) {
                if ( isset( $cred['id'] ) && $cred['id'] == $id ) {
                    $cred = $new_data;
        ...
        $auth_url = add_query_arg(
            array(
                ...
                'redirect_uri'  => $this->get_redirect_uri(),
                'scope'         => $scope,
                'state'         => self::issue_oauth_state( 'bigin', $id ),
            ),
            $auth_endpoint
        );
    ...
    public function auth_redirect() {
        ...
        $context = self::consume_oauth_state( $state, 'bigin' );
    ...
    protected function request_token( $authorization_code ) {
        ...
        // Basic Auth is rejected with invalid_client.
        $request = array(
            'timeout' => 30,
            'body'    => array(
        ...
        <script type='text/template' id='bigin-action-template'>
            <table class='form-table'>
                ...
                <tr valign='top' class='alternate' v-if="action.task == 'subscribe'">
                    <td scope='row-title'>
                        <label for='tablecell'>
                            <?php esc_attr_e( 'Bigin Account', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                        ...
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=bigin' ); ?>" 
                           target="_blank" 
                ...
                <?php adfoin_pro_feature_notice( 'subscribe', 'Bigin [PRO]', 'custom fields' ); ?>
            </table>
        </script>
    ...
    public function bigin_request($endpoint, $method = 'GET', $data = array(), $record = array() ) {
...
protected function remote_request( $url, $request = array(), $record = array() ) {
    ...
    */
    function get_fields() {
        ...
        if( $module ) {
            ...
            if( isset( $body['fields'] ) && is_array( $body['fields'] ) ) {
                ...
                $suppression_list = array( 'Created_By', 'Modified_By', 'Created_Time', 'Modified_Time', 'Layout', 'Tag', 'Recurring_Activity', 'BEST_TIME', 'What_Id', 'Record_Image' );
    ...
    public function search_record( $module, $search_key, $search_value, $record ) {
        ...
        // Escape single quotes so values like O'Brien don't break the COQL.
        $escaped_value = str_replace( "'", "\\'", (string) $search_value );
        ...
        for ( $i = 0; $i < $attempts; $i++ ) {
            $response = $this->bigin_request( 'coql', 'POST', $body, $record );
            ...
            // Last attempt: don't sleep, return empty.
...
add_action( 'adfoin_bigin_job_queue', 'adfoin_bigin_job_queue', 10, 1 );
        ...
        foreach ( $data as $key => $value ) {
            ...
            if( 'bigint' == $data_type && $value ) {
                if( 'Participants' == $original_key ) {
                    ...
                    foreach( $raw_participants as $single ) {
                        ...
                        if( 'contact' == $type ) {
                            $participant_id = $bigin->search_record( 'Contacts', 'Email', $email, $record )['id'];
            ...
            if( 'lookup' == $data_type && $value ) {
                ...
                if( in_array( $original_key, $contact_lookups ) ) {
                    ...
                    $contact_id = $bigin->search_record( 'Contacts', 'Email', $value, $record )['id'];

                    if( ! $contact_id ) {
                        // Auto-create when missing so the deal isn't left
                        // unlinked. Mirrors the Pro behavior for Accounts.
                        $local      = strstr( $value, '@', true );
                        $last_name  = $local ?: $value;
                        ...
                        $return     = $bigin->bigin_request( 'Contacts/upsert', 'POST', $contact_payload, $record );
                        $body       = json_decode( wp_remote_retrieve_body( $return ), true );
                        $contact_id = isset( $body['data'], $body['data'][0], $body['data'][0]['details'], $body['data'][0]['details']['id'] ) ? $body['data'][0]['details']['id'] : '';
                    }
                ...
                if( in_array( $original_key, $campaign_lookups ) ) {
                    $campaign_id = $bigin->search_record( 'Campaigns', 'Campaign_Name', $value, $record )['id'];
                ...
                if( 'Product_Name' == $original_key ) {
                    $product_id = $bigin->search_record( 'Products', 'Product_Name', $value, $record )['id'];
