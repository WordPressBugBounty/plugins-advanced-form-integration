...
class ADFOIN_ZohoMA extends Advanced_Form_Integration_OAuth2 {
    ...
    protected $platform_slug = 'zohoma';
    ...
    public function __construct() {
        ...
        add_filter( 'adfoin_action_providers', array( $this, 'adfoin_zohoma_actions' ), 10, 1 );
        add_filter( 'adfoin_settings_tabs', array( $this, 'adfoin_zohoma_settings_tab' ), 10, 1 );
        add_action( 'adfoin_settings_view', array( $this, 'adfoin_zohoma_settings_view' ), 10, 1 );
        add_action( 'adfoin_action_fields', array( $this, 'action_fields' ), 10, 1 );
        add_action( 'wp_ajax_adfoin_get_zohoma_lists', array( $this, 'get_lists' ), 10, 0 );
        ...
        add_action( 'wp_ajax_adfoin_get_zohoma_credentials', array( $this, 'get_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_save_zohoma_credentials', array( $this, 'save_credentials' ), 10, 0 );
        add_action( 'wp_ajax_adfoin_test_zohoma_connection', array( $this, 'test_connection' ), 10, 0 );
    }

    public function create_webhook_route() {
        register_rest_route( 'advancedformintegration', '/zohoma',
            array(
                'methods'             => 'GET',
    ...
    public function get_webhook_data( $request ) {
        ...
        $context = self::consume_oauth_state( $state, 'zohoma' );
    ...
    public function test_connection() {
        $this->run_test_connection_ajax( function () {
            return $this->zohoma_request( 'getmailinglists?resfmt=JSON&range=1' );
        } );
    }
    ...
    */
    function save_credentials() {
        ...
        foreach ( $credentials as &$cred ) {
            if ( $cred['id'] == $id ) {
                if (
                    isset( $cred['client_id'] ) && $cred['client_id'] == $client_id &&
        ...
        $auth_url = add_query_arg( array(
            ...
            'scope'         => $scope,
            'state'         => self::issue_oauth_state( 'zohoma', $id )
        ), $auth_endpoint );
    ...
    public function adfoin_zohoma_settings_view( $current_tab ) {
        ...
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-oauth-manager.php';
        ADFOIN_OAuth_Manager::render_oauth_settings_view( 'zohoma', 'Zoho Marketing Automation', $fields, $instructions, $config );
    }
    ...
    public function action_fields() {
        ...
        <script type='text/template' id='zohoma-action-template'>
            <table class='form-table'>
                ...
                <?php adfoin_pro_feature_notice( 'subscribe', 'Zoho Marketing Automation [PRO]', 'custom fields' ); ?>
            </table>
        </script>
    ...
    public function zohoma_request( $endpoint, $method = 'GET', $data = array(), $record = array() ) {
    ...
    */
    function get_fields() {
        ...
        if( isset( $body['response'], $body['response']['fieldnames'], $body['response']['fieldnames']['fieldname'] ) && is_array( $body['response']['fieldnames']['fieldname'] ) ) {
            foreach( $body['response']['fieldnames']['fieldname'] as $field ) {
                if( in_array( $field['FIELD_NAME'], array( 'do_not_call', 'do_not_call_reason', 'contact_owner', 'preferred_language', 'total_sales_activity' ) ) ) {
                    continue;
                }
                ...
                if( 'standard' == $field['TYPE'] ) {
                    ...
                    array_push( $lead_fields, array( 'key' => $field['UITYPE'] . '__' . $field['DISPLAY_NAME'], 'value' => $field['DISPLAY_NAME'], 'description' => $description ) );
                }
            }
        }
    ...
    public function create_lead( $lead_data, $record ) {
        ...
        $response = $this->zohoma_request( 'json/listsubscribe', 'POST', $lead_data, $record );
        $body     = json_decode( wp_remote_retrieve_body( $response ), true );
...
add_action( 'adfoin_zohoma_job_queue', 'adfoin_zohoma_job_queue', 10, 1 );
