...
add_filter( 'adfoin_action_providers', 'adfoin_emailoctopus_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_emailoctopus_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_emailoctopus_settings_view', 10, 1 );
...
function adfoin_emailoctopus_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        '<p><a href="https://emailoctopus.com/api-documentation/v2" target="_blank" rel="noopener noreferrer">%s</a></p>',
        __('Click here to the get API Key', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'emailoctopus', __( 'EmailOctopus', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_emailoctopus_credentials', 'adfoin_get_emailoctopus_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_emailoctopus_credentials', 'adfoin_save_emailoctopus_credentials', 10, 0 );
/*
...
// Legacy single-account import: surfaces old `adfoin_emailoctopus_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'emailoctopus', array(
            'apiKey' => 'adfoin_emailoctopus_api_key',
        ) );
    }
}, 20 );

// Deprecated - kept for backward compatibility
add_action( 'admin_post_adfoin_save_emailoctopus_api_key', 'adfoin_save_emailoctopus_api_key', 10, 0 );
...
function adfoin_save_emailoctopus_api_key() {
    ...
    $api_key = sanitize_text_field( wp_unslash( $_POST['adfoin_emailoctopus_api_key'] ) );
 ...
 */
function adfoin_emailoctopus_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
add_action( 'adfoin_add_js_fields', 'adfoin_emailoctopus_js_fields', 10, 1 );
...
function adfoin_emailoctopus_action_fields() {
    ...
    <script type="text/template" id="emailoctopus-action-template">
        <table class="form-table">
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'EmailOctopus [PRO]', 'custom fields and update contact' ); ?>
        </table>
    </script>
...
add_action( 'wp_ajax_adfoin_get_emailoctopus_list', 'adfoin_get_emailoctopus_list', 10, 0 );
...
function adfoin_emailoctopus_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        if( 'true' == $doubleoption ) {
            $subscriber_data['subscribed'] = false; // Will send confirmation email
        } else {
