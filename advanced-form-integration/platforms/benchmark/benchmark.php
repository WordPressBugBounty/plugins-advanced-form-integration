...
add_filter( 'adfoin_action_providers', 'adfoin_benchmark_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_benchmark_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_benchmark_settings_view', 10, 1 );
...
function adfoin_benchmark_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Go to Profile > Integrations > API Key and copy the key', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'benchmark', __( 'Benchmark', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_benchmark_credentials', 'adfoin_get_benchmark_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_benchmark_credentials', 'adfoin_save_benchmark_credentials', 10, 0 );
/*
...
// Legacy single-account import: surfaces old `adfoin_benchmark_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'benchmark', array(
            'apiKey' => 'adfoin_benchmark_api_key',
        ) );
    }
}, 20 );

// Deprecated - kept for backward compatibility
add_action( 'admin_post_adfoin_benchmark_save_api_key', 'adfoin_save_benchmark_api_key', 10, 0 );
...
function adfoin_save_benchmark_api_key() {
    ...
    $api_key = sanitize_text_field( wp_unslash( $_POST["adfoin_benchmark_api_key"] ) );
...
add_action( 'adfoin_add_js_fields', 'adfoin_benchmark_js_fields', 10, 1 );
    ...
    <script type="text/template" id="benchmark-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Benchmark Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=benchmark' ); ?>" target="_blank">
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'Benchmark [PRO]', 'custom fields' ); ?>
...
function adfoin_benchmark_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_benchmark_list', 'adfoin_get_benchmark_list', 10, 0 );
...
function adfoin_benchmark_create_contact( $list_id, $properties, $record, $cred_id = '' ) {
    $endpoint = "Contact/{$list_id}/ContactDetails";
    $response = adfoin_benchmark_request( $endpoint, 'POST', $properties, $record, $cred_id);
    
    return $response;
}

function adfoin_benchmark_update_contact(  $list_id, $contact_id, $properties, $record, $cred_id = '' ) {
    $endpoint = "Contact/{$list_id}/ContactDetails/{$contact_id}";
    $response = adfoin_benchmark_request( $endpoint, 'PATCH', $properties, $record, $cred_id);
...
function adfoin_benchmark_check_if_contact_exists( $email, $cred_id = '' ){
    ...
    if( isset( $body['Response'], $body['Response']['Data'], $body['Response']['Data'][0], $body['Response']['Data'][0]['ID'] ) ) {
 ...
 add_action( 'adfoin_benchmark_job_queue', 'adfoin_benchmark_job_queue', 10, 1 );
...
function adfoin_benchmark_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        if($contact_id){
            $return = adfoin_benchmark_update_contact( $list_id, $contact_id, $data, $record, $cred_id );
        }else{
            $return = adfoin_benchmark_create_contact( $list_id, $data, $record, $cred_id );
        }        
    }
