...
function adfoin_attio_settings_view( $current_tab ) {
    ...
    $fields = array(
        array(
            ...
            'required'      => true,
            'mask'          => true,
            'placeholder'   => __( 'Paste your Attio access token', 'advanced-form-integration' ),
            'show_in_table' => true,
        ),
    );
    ...
    ADFOIN_Account_Manager::render_settings_view(
        'attio',
        __( 'Attio CRM', 'advanced-form-integration' ),
        $fields,
        $instructions
    );
}
    ...
    <script type='text/template' id='attio-action-template'>
            <table class='form-table'>
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Attio Account', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                        ...
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=attio' ); ?>" target="_blank">
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Update Existing Record', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                ...
                <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                    <td scope="row-title">
                        <label for="tablecell">
                            <?php esc_attr_e( 'Matching Attribute', 'advanced-form-integration' ); ?>
                        </label>
                    </td>
                    <td>
                        <input class="regular-text" type="text" name="fieldData[match]" v-model="fielddata.match">
                        <p class="description"><?php esc_attr_e( 'Only required for updating existing records. Example: domains, email_addresses, name.', 'advanced-form-integration' ); ?></p>
                    </td>
                </tr>
                ...
                <?php adfoin_pro_feature_notice( 'subscribe', 'Attio CRM [PRO]', 'additional objects' ); ?>
            </table>
        </script>
    <?php
}
 ...
 * Attio API Request
 */
function adfoin_attio_request( $endpoint, $method = 'GET', $data = [], $record = [], $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_attio_credentials', 'adfoin_get_attio_credentials', 10, 0 );
/*
 * Get Attio credentials (masked list for action dropdowns).
 */
function adfoin_get_attio_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'attio' );
}

add_action( 'wp_ajax_adfoin_save_attio_credentials', 'adfoin_save_attio_credentials', 10, 0 );
/*
 * Save (or delete) a single Attio account row.
 */
function adfoin_save_attio_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'attio', array( 'accessToken' => 'token' ) );
}

add_action( 'wp_ajax_adfoin_get_attio_object_fields', 'adfoin_get_attio_object_fields', 10, 0 );
/*
...
function adfoin_get_attio_object_fields() {
    ...
    $data      = adfoin_attio_request( "objects/{$object_id}/attributes", 'GET', [], [], $cred_id );
        ...
        foreach ( $response_body['data'] as $single ){
            if( true == $single['is_writable'] && !in_array( $single['api_slug'], $skip ) ) {
                ...
                if( 'personal-name' == $single['type'] && 'name' == $single['api_slug'] ) {
                    ...
                    array_push( $fields, [ 'key' => $prefix . '__' . $single['type'] . '__' . $single['api_slug'], 'value' => $single['title'], 'description' => '' ] );
                }
            }
        }
    } else {
...
add_action( 'wp_ajax_adfoin_get_attio_objects', 'adfoin_get_attio_objects', 10, 0 );
/*
 * Get Attio onjects
 */
...
add_action( 'adfoin_attio_job_queue', 'adfoin_attio_job_queue', 10, 1 );
/*
 * Attio Job Queue
 */
...
function adfoin_attio_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        $request_data = [ 'data' => [ 'values' => $request_data ] ];
        ...
        if( $update == 'true' ) {
            ...
            $return = adfoin_attio_request( "objects/{$object_id}/records?matching_attribute={$matching_attribute}", 'PUT', $request_data, $record, $cred_id );
        } else {
            $return = adfoin_attio_request( "objects/{$object_id}/records", 'POST', $request_data, $record, $cred_id );
        }
    }
...
function adfoin_attio_search_record( $object_id, $term, $cred_id ) {
    ...
    $result = adfoin_attio_request( "objects/{$object_id}/records/query", 'POST', $serarch_term, [], $cred_id );
    $body = json_decode( wp_remote_retrieve_body( $result ), true );

    if( isset( $body['data'], $body['data'][0], $body['data'][0]['id'], $body['data'][0]['id']['record_id'] )  ) {
