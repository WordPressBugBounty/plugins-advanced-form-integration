...
add_filter( 'adfoin_action_providers', 'adfoin_insightly_actions', 10, 1 );
...
add_filter( 'adfoin_settings_tabs', 'adfoin_insightly_settings_tab', 10, 1 );
...
add_action( 'adfoin_settings_view', 'adfoin_insightly_settings_view', 10, 1 );
...
function adfoin_insightly_settings_view( $current_tab ) {
    ...
    $fields = array(
        ...
        array( 
            ...
            'type' => 'text', 
            'required' => true,
            'placeholder' => __( 'Enter your API URL', 'advanced-form-integration' ),
            'show_in_table' => true
        )
    );
    ...
    $instructions = sprintf(
        '<p>%s</p>',
        __('Go to User Settings to get the API Key and URL', 'advanced-form-integration')
    );

    ADFOIN_Account_Manager::render_settings_view( 'insightly', __( 'Insightly', 'advanced-form-integration' ), $fields, $instructions );
}

add_action( 'wp_ajax_adfoin_get_insightly_credentials', 'adfoin_get_insightly_credentials', 10, 0 );
/*
 * Get Insightly credentials
 */
function adfoin_get_insightly_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_get_credentials_list( 'insightly' );
}

add_action( 'wp_ajax_adfoin_save_insightly_credentials', 'adfoin_save_insightly_credentials', 10, 0 );
/*
 * Save Insightly credentials
 */
function adfoin_save_insightly_credentials() {
    if ( ! class_exists( 'ADFOIN_Account_Manager' ) ) {
        require_once plugin_dir_path( __FILE__ ) . '../../includes/class-adfoin-account-manager.php';
    }
    ADFOIN_Account_Manager::ajax_save_credentials( 'insightly', array( 'apiKey', 'apiUrl' ) );
}

/*
 * Insightly Credentials List
 */
function adfoin_insightly_get_credentials_list() {
    $credentials = adfoin_read_credentials( 'insightly' );
...
// Legacy single-account import: surfaces old `adfoin_insightly_*` options
// as a Legacy Account record when the new credentials store is empty.
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'ADFOIN_Account_Manager' ) ) {
        ADFOIN_Account_Manager::register_legacy_option_importer( 'insightly', array(
            'apiKey' => 'adfoin_insightly_api_key',
            'apiUrl' => 'adfoin_insightly_api_url',
        ) );
    }
}, 20 );

// Deprecated - kept for backward compatibility
add_action( 'admin_post_adfoin_save_insightly_api_key', 'adfoin_save_insightly_api_key', 10, 0 );
...
function adfoin_save_insightly_api_key() {
    ...
    $api_key = sanitize_text_field( wp_unslash( $_POST["adfoin_insightly_api_key"] ) );
    $api_url = sanitize_text_field( wp_unslash( $_POST["adfoin_insightly_api_url"] ) );
...
add_action( 'adfoin_add_js_fields', 'adfoin_insightly_js_fields', 10, 1 );
    ...
    <script type="text/template" id="insightly-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'add_contact'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Insightly Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
            ...
            <?php adfoin_pro_feature_notice( 'add_contact', 'Insightly [PRO]', 'custom fields and tags' ); ?>
        </table>
    </script>
...
function adfoin_insightly_get_keys( $cred_id = '' ) {
    $credentials = adfoin_get_credentials_by_id( 'insightly', $cred_id );
    $key = isset( $credentials['apiKey'] ) ? $credentials['apiKey'] : '';
    $url = isset( $credentials['apiUrl'] ) ? $credentials['apiUrl'] : '';

    // Backward compatibility: fallback to old options if credentials not found
    if( empty( $key ) || empty( $url ) ) {
        $key = get_option( 'adfoin_insightly_api_key' );
        $url = get_option( 'adfoin_insightly_api_url' );
    }

    return array( 'key' => $key, 'url' => $url );
}

add_action( 'wp_ajax_adfoin_get_insightly_owner_list', 'adfoin_get_insightly_owner_list', 10, 0 );
...
function adfoin_insightly_get_pipelines( $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_insightly_all_fields', 'adfoin_get_insightly_all_fields', 10, 0 );
...
function adfoin_get_insightly_all_fields() {
    ...
    // $pipelines = adfoin_insightly_get_pipelines( $cred_id );
...
function adfoin_insightly_send_data( $record, $posted_data ) {
    ...
    if( empty( $cred_id ) ) {
        $all_credentials = adfoin_read_credentials( 'insightly' );
