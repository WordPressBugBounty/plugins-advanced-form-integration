Vue.component('marketo', {
    ...
    methods: {
        fetchCredentialsList: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_marketo_credentials', _nonce: adfoin.nonce }, function (response) {
        ...
        fetchFields: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_marketo_fields', _nonce: adfoin.nonce }, function (response) {
                that.fieldsLoading = false;
                if (response && response.success && Array.isArray(response.data)) {
                    that.fields = response.data.map(function (single) {
                        return { type: 'text', value: single.key, title: single.value, task: ['sync_lead'], required: !!single.required };
                    });
                } else {
                    that.fields = [];
                }
