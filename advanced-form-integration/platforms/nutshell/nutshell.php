...
class ADFOIN_Nutshell {
    ...
    public function __construct() {
        ...
        add_filter('adfoin_action_providers', array($this, 'adfoin_nutshell_actions'), 10, 1);
        add_filter('adfoin_settings_tabs', array($this, 'adfoin_nutshell_settings_tab'), 10, 1);
        add_action('adfoin_settings_view', array($this, 'adfoin_nutshell_settings_view'), 10, 1);
    ...
    public function get_credentials_list_ajax() {
        ...
        $fields = array(
            array( 'name' => 'user_email', 'mask' => false ),
        ...
        <script type='text/template' id='nutshell-action-template'>
            <table class='form-table'>
                <tr valign='top' v-if="action.task == 'add_contact'">
                    <th scope='row'><?php esc_html_e( 'Nutshell Account', 'advanced-form-integration' ); ?></th>
                    <td>
                        <select name="fieldData[credId]" v-model="fielddata.credId" @change="getOwners">
                            <option value=""> <?php _e( 'Select Account...', 'advanced-form-integration' ); ?> </option>
                            <option v-for="cred in credentialsList" :value="cred.id">{{ cred.title }}</option>
                        </select>
                        <a href="<?php echo admin_url( 'admin.php?page=advanced-form-integration-settings&tab=nutshell' ); ?>" 
                           target="_blank" 
                ...
                <?php adfoin_pro_feature_notice( 'add_contact', 'Nutshell [PRO]', 'custom fields' ); ?>
            </table>
        </script>
        <?php
    }
    ...
    public function adfoin_nutshell_settings_view($current_tab) {
        if ($current_tab !== 'nutshell') return;
        ...
        ADFOIN_Account_Manager::render_settings_view( 'nutshell', 'Nutshell CRM', $fields, $instructions );
    }
    ...
    public function get_fields() {
        ...
        if ($task == 'add_contact') {
            $account_fields = [
                ['key' => 'account__name', 'value' => 'Name [Account]'],
                ['key' => 'account__description', 'value' => 'Description [Account]'],
                ['key' => 'account__phone', 'value' => 'Phone [Account]'],
                ['key' => 'account__email', 'value' => 'Email [Account]'],
                ['key' => 'account__url', 'value' => 'URL [Account]'],
                ['key' => 'account__address_1', 'value' => 'Address 1 [Account]'],
                ['key' => 'account__city', 'value' => 'City [Account]'],
                ['key' => 'account__state', 'value' => 'State [Account]']
            ];

            $contact_fields = [
                ['key' => 'contact__name', 'value' => 'Name [Contact]'],
                ['key' => 'contact__email', 'value' => 'Email [Contact]'],
                ['key' => 'contact__bio', 'value' => 'Bio [Contact]'],
                ['key' => 'contact__phone', 'value' => 'Phone [Contact]'],
                ['key' => 'contact__address_1', 'value' => 'Address 1 [Contact]'],
                ['key' => 'contact__city', 'value' => 'City [Contact]'],
                ['key' => 'contact__state', 'value' => 'State [Contact]'],
                ['key' => 'contact__postalCode', 'value' => 'Postal Code [Contact]'],
                ['key' => 'contact__country', 'value' => 'Country [Contact]'],
                ['key' => 'contact__url', 'value' => 'URL [Contact]'],
            ];
    ...
    public function create_account($account_data, $record, $cred_id = '') {
        $response = $this->request('accounts', 'POST', $account_data, $record, $cred_id);
    ...
    public function find_account($name, $cred_id = '') {
        ...
        $response = $this->request($endpoint, 'GET', array(), array(), $cred_id);
    ...
    public function update_account($account_id, $account_data, $record, $cred_id = '') {
        $patch_data = [];
        ...
        $response = $this->request($endpoint, 'PATCH', $patch_data, $record, $cred_id);
        return $response;
    }

    public function create_contact($contact_data, $record, $cred_id = '') {
        $response = $this->request('contacts', 'POST', $contact_data, $record, $cred_id);
        $response_body = json_decode(wp_remote_retrieve_body($response), true);
    ...
    public function update_contact($contact_id, $contact_data, $record, $cred_id = '') {
        $endpoint = 'contacts/' . $contact_id;
        $response = $this->request($endpoint, 'PUT', $contact_data, $record, $cred_id);

        return $response;
    }

    public function request($endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '') {
...
add_action('adfoin_nutshell_job_queue', 'adfoin_nutshell_job_queue', 10, 1);
    ...
    if ($task == 'add_contact') {
        ...
        if (!empty($parsed_account_data)) {
            ...
            if ($existing_account) {
                ...
                $nutshell->update_account($account_id, $account_data, $record, $cred_id);
            } else {
        ...
        if (!empty($parsed_contact_data)) {
            ...
            if ($existing_contact) {
                ...
                $nutshell->update_contact($contact_id, $contact_data, $record, $cred_id);
            } else {
