...
function adfoin_flodesk_settings_view( $current_tab ) {
    ...
    $instructions = sprintf(
        ...
        __('Go to Profile > Integrations > API Keys.', 'advanced-form-integration'),
        __('Crate API Key and copy.', 'advanced-form-integration')
    );
    ...
    <script type="text/template" id="flodesk-action-template">
        <table class="form-table">
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Flodesk Account', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    ...
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=advanced-form-integration-settings&tab=flodesk' ) ); ?>" target="_blank" style="margin-left: 10px; text-decoration: none;"><span class="dashicons dashicons-admin-settings" style="margin-top: 3px;"></span> <?php esc_html_e( 'Manage Accounts', 'advanced-form-integration' ); ?></a>
                </td>
            </tr>
            ...
            <tr valign="top" class="alternate" v-if="action.task == 'subscribe'">
                <td scope="row-title">
                    <label for="tablecell">
                        <?php esc_attr_e( 'Flodesk Segment', 'advanced-form-integration' ); ?>
                    </label>
                </td>
                <td>
                    <select name="fieldData[segmentId]" v-model="fielddata.segmentId">
                        <option value=""> <?php _e( 'Select Segment...', 'advanced-form-integration' ); ?> </option>
                        <option v-for="(item, index) in fielddata.segments" :value="index" > {{item}}  </option>
                    </select>
                    <div class="afi-spinner" v-bind:class="{'is-active': segmentsLoading}"></div>
                </td>
            </tr>
            ...
            <?php adfoin_pro_feature_notice( 'subscribe', 'Flodesk [PRO]', 'custom fields' ); ?>
 ...
 * flodesk API Request
 */
function adfoin_flodesk_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
add_action( 'wp_ajax_adfoin_get_flodesk_credentials', 'adfoin_get_flodesk_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_save_flodesk_credentials', 'adfoin_save_flodesk_credentials', 10, 0 );
/*
...
add_action( 'wp_ajax_adfoin_get_flodesk_segments', 'adfoin_get_flodesk_segments', 10, 0 );
/*
...
function adfoin_get_flodesk_segments() {
    ...
    do {
        $data = adfoin_flodesk_request( 'segments?page=' . $page . '&per_page=' . $per_page, 'GET', array(), array(), $cred_id );
...
add_action( 'adfoin_flodesk_job_queue', 'adfoin_flodesk_job_queue', 10, 1 );
...
function adfoin_flodesk_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        $return = adfoin_flodesk_request( 'subscribers', 'POST', $subscriber_data, $record, $cred_id );

        if( !is_wp_error( $return ) && !empty( $segment_id ) ) {
            adfoin_flodesk_request( "subscribers/{$subscriber_data['email']}/segments", 'POST', array('segment_ids' => array($segment_id)), $record, $cred_id );
        }
    }
