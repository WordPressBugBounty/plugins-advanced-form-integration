Vue.component('greenhouse', {
    ...
    methods: {
        fetchCredentialsList: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_greenhouse_credentials', _nonce: adfoin.nonce }, function (response) {
        ...
        fetchFields: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_greenhouse_fields', _nonce: adfoin.nonce }, function (response) {
                that.fieldsLoading = false;
                if (response && response.success && Array.isArray(response.data)) {
                    that.fields = response.data.map(function (single) {
                        return { type: 'text', value: single.key, title: single.value, task: ['submit_application'], required: !!single.required, description: single.description || '' };
                    });
                } else {
                    that.fields = [];
                }
    ...
    mounted: function () {
        ...
        if (typeof this.fielddata.jobId == 'undefined') this.fielddata.jobId = '';
        this.fetchCredentialsList();
        this.fetchFields();
    },
    template: '#greenhouse-action-template'
});
