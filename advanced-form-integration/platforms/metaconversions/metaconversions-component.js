Vue.component('metaconversions', {
    ...
    methods: {
        fetchCredentialsList: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_metaconversions_credentials', _nonce: adfoin.nonce }, function (response) {
        ...
        fetchFields: function () {
            ...
            jQuery.post(ajaxurl, { action: 'adfoin_get_metaconversions_fields', _nonce: adfoin.nonce }, function (response) {
                that.fieldsLoading = false;
                if (response && response.success && Array.isArray(response.data)) {
                    that.fields = response.data.map(function (single) {
                        return { type: single.type || 'text', value: single.key, title: single.value, task: ['send_event'], required: !!single.required, description: single.description || '' };
                    });
                } else {
                    that.fields = [];
                }
    ...
    template: '#metaconversions-action-template'
});
